...
export function EntregarForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!tareaId || !archivo || enviando) return
    setEnviando(true)
    setError(null)
    ...
    const path = `${tareaId}/${user.id}/${sanitizeFileName(archivo.name)}`
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button type="submit" disabled={!archivo || !tareaId || enviando} className={btnPrimary}>
          {enviando && (
            <span className="size-4 animate-spin rounded-full border-2 border-paper-raised/40 border-t-paper-raised" />
          )}
          {enviando ? 'Subiendo…' : tareaSeleccionada?.entrega ? 'Reemplazar entrega' : 'Subir entrega'}
        </button>
        <Link
